...
window.onerror = (msg, url, lineNo, columnNo, error) => {
  try {
    ...
    element.onclick = () => {element.parentNode.removeChild(element)}
    $('#error-stack').appendChild(element)
    sound.playMenuSe('error')
  } catch (e) {console.log(e)} // We don't want an infinite loop of errors
}
document.addEventListener('DOMContentLoaded', () => {
  ...
  locale.loadLang(settings.settings.language)
    .then((test) => {
      ...
      window.onblur = () => {
        try {gameHandler.game.pause()}
        catch (e) {} // game isn't running yet no big deal
        finally {
      ...
      window.onfocus = () => {Howler.volume(1)}
    })
})
