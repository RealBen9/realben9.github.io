...
export default class Game {
  constructor(gametype, seed) {
    ...
    loadGameType(gametype)
        .then((gameData) => {
          ...
          if (this.type === 'nontwo') {$('#lights-warning').classList.remove('hidden')}
          else {$('#lights-warning').classList.add('hidden')}

          for (const element of document.querySelectorAll('.action-text')) {element.parentNode.removeChild(element)}
          ...
          window.inputList = []
          let randomseed = seed || new Math.seedrandom()()
          //console.log("Seed : " + randomseed)
          if ($('#queuerand').value !== '') {randomseed = $('#queuerand').value}
          ...
          if (this.userSettings.rotationSystem === 'auto') {this.rotationSystem = this.settings.rotationSystem}
          else {
          ...
          if (!this.settings.disableDefaultSkinLoad) {this.makeSprite()}
          const soundbankName = (settings.settings.soundbank === 'auto') ? SOUND_SETS[this.settings.rotationSystem] : settings.settings.soundbank
          ...
          for (const element of ['piece', 'stack', 'next', 'hold']) {
            if (gameData[element] != null) {
              for (const property of Object.keys(gameData[element])) {this[element][property] = gameData[element][property]}
            }
          }
          this.resize()
          loops[gametype].onInit(this)

          if (!this.userSettings.useAre) {this.piece.areLimit = 0}
          ...
          if (gametype === 'beat'){this.settings.music = settings.game.beat.song}

          if (this.settings.musicLinePoints != null) {
            // eslint-disable-next-line no-unused-vars
            for (const point of this.settings.musicLinePoints) {this.musicLinePointCleared.push(false)}
          }
          ...
          if (sound.doesSoundBankUseReadyGoVoices && settings.settings.voicebank != 'off' && settings.settings.voiceVolume != 0) {playSoundBankReadyGoSoundOrVoice = false}

          if (playSoundBankReadyGoSoundOrVoice) {sound.add('ready')}
          sound.add('voxready')
  ...
  die() {
    ...
    if (victory == true || gameHandler.game.lives <= 0) {
      ...
      for (const statName of this.stats) {
        ...
        if (this.endingStats[statName] && !"b2b piece pcCount skipCount".includes(statName)) {$('#end-stats').innerHTML += `<b>${locale.getString('ui', statName)}:</b> ${this.stat[statName]}${append}<br>`}
        switch(statName){
          case 'b2b':
      ...
      if (victory) {sound.add('excellent')}
      else {
        if (!(settings.settings.soundbank === 't99' && settings.settings.voicebank !== 'off')) {sound.add('ko')}
      }
      sound.killBgm()
      sound.killAllLoops()
      $('#game').classList.add('dead')
      window.replay = Date.now() + "\nNAME\n" + this.type + "\n" + JSON.stringify(settings.game[this.type]) + "\n" + settings.settings.ARR + "\n" + settings.settings.DAS + "\n" + settings.settings.IAS + "\n" + settings.settings.IHS + "\n" + settings.settings.IRS + "\n" + settings.settings.nextLength + "\n" + settings.settings.rotationSystem + "\n" + gameHandler.game.next.seed + "\n" + JSON.stringify(window.inputList) + "\n" + $('#end-stats').innerText
      endScreenTimeout = setTimeout(() => {
        ...
        if (this.type === 'handheld' || this.type === 'deluxe') {$('#end-message').innerHTML = `${locale.getString('ui', 'gameover')}<br><span class="small">${locale.getString('ui', 'pleasetryagain')}♥</span>`}
  ...
  calculateActionText(lineClear, isSpin, isMini, b2b, isClutch) {
    if (!settings.settings.displayActionText) {return}
  ...
  displayActionText(text, options) {
    ...
    if (options.skipDefaultAnimation) {element.classList.add('skip-default-animation')}
    for (const className of options.additionalClasses) {element.classList.add(className)}
  ...
  resize() {
    ...
    for (const statName of game.stats) {
      ...
      if (!game.smallStats[statName]) {number.classList.add('big')}
      stat.appendChild(label)
      stat.appendChild(number)
      $('#stats').appendChild(stat)
    }
    if (game.stack.width <= 4 && (settings.settings.language === 'ja_JP' || settings.settings.language === 'ko_KR')) {$('#pause-label').classList.add('vertical')}
    else {$('#pause-label').classList.remove('vertical')}
    game.updateStats()
  }
  drawLockdown() {
    $('#pip-grid').innerHTML = ''
    ...
    switch (this.piece.lockdownType) {
      case 'extended':
        ...
        $('#lockdown').classList.remove('strict')
  ...
  updateStats() {
    for (const statName of this.stats) {
      ...
      if (statName === 'piece'){$('#stat-piece').innerHTML = `<span class="medium">${value}</span><br><b>${Math.round(gameHandler.game.pps * 100) / 100}</b>/${locale.getString('ui', 'sec')}`}
      if (statName === 'b2b'){$('#stat-b2b').innerHTML = `×${value}<br>(Max: ×${this.maxb2b-1 < 0 ? 0 : this.maxb2b-1})`}
    }
  }
  shiftMatrix(direction) {
    if (settings.settings.matrixSwayScale <= 0) {return}
    switch (direction) {
      case 'left':
  ...
  updateMatrix(ms) {
    ...
    for (const direction of ['x', 'y']) {
      if (Math.abs(this.matrix.position[direction]) < 0.0001) {this.matrix.position[direction] = 0}
    }
    for (const directions of [['left', 'right', 'x'], ['up', 'down', 'y']]) {
      if (
        ...
        for (let i = 0; i < 2; i++) {
          ...
          if (this.matrix.velocity[direction] !== 0) {matrixPush(direction)}
        }
      }
    }
    for (const direction of ['x', 'y']) {
  ...
  updateMusic() {
    if (this.settings.musicLinePoints != null) {
      for (let i = 0; i < this.musicLinePointCleared.length; i++) {
        if (!this.musicLinePointCleared[i]) {
    ...
    if (!game.isDead) {
      ...
      if (typeof game.loop === 'function') {
        ...
        if (!game.isPaused) {
          ...
          if (!game.noUpdate) {
            if (!game.piece.inAre) {game.timePassed += msPassed}
            else if (game.piece.startingAre >= game.piece.startingAreLimit) {game.timePassedAre += msPassed}
          ...
          for (const moduleName of modules) {
            ...
            if (currentModule.isDirty || game.isDirty) {
              if (moduleName === 'stack' && game.isDirty) {game.stack.makeAllDirty()}
              currentModule.draw()
        ...
        if (game.piece.lockdownTypeLast !== game.piece.lockdownType) {game.drawLockdown()}
        ...
        if (input.getGamePress('retry')) {game.mustReset = true}
        sound.playSeQueue()
        input.updateGameInput()
        if (game.mustReset) {game.isDead = true}
        if (game.isPaused) {
          $('#timer').classList.add('paused')
          $('#timer-real').classList.add('paused')
        } else {
          if (game.piece.inAre) {$('#timer').classList.add('paused')}
          else {$('#timer').classList.remove('paused')}
          if (game.piece.startingAre < game.piece.startingAreLimit) {$('#timer-real').classList.add('paused')}
          else {$('#timer-real').classList.remove('paused')}
        }
        if (game.timeGoal != null) {
          if (game.rtaLimit) {
      ...
      if (game.mustReset) {gameHandler.reset()}
    }
  }
  makeSprite(
      colors = ['red', 'orange', 'yellow', 'green', 'lightBlue', 'blue', 'purple', 'white', 'black'],
      types = ['mino', 'ghost', 'stack'],
      skin = (settings.settings.skin === 'auto') ? SKIN_SETS[this.settings.rotationSystem] : settings.settings.skin,
  ) {
    if (gameHandler.game.stat.level >= 1000 && gameHandler.game.type == 'suddenti') {skin = 'bone'}
    this.loadFinished = false
    $('#sprite').innerHTML = ''
  ...
  addScore(name, multiplier = 1) {
    ...
    if (score != null) {
      score *= multiplier
      if (scoreTable.levelMultiplied.indexOf(name) !== -1) {score *= this.stat.level + scoreTable.levelAdditive}
      if (scoreTable.b2bMultiplied.indexOf(name) !== -1 && this.b2b > 1) {score *= scoreTable.b2bMultiplier}
