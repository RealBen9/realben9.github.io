...
export default class Piece extends GameModule {
  ...
  new(name = this.parent.next.next()) {
    ...
    $('#delay').innerHTML = `${this.lockDelayLimit}/${this.lockDelayLimit} <b>ms</b>`
    $('#delay').classList.remove('lowdelay')
    $('#delay').classList.remove('lowmiddelay')
    $('#delay').classList.remove('middelay')
    $('#delay').style.fontSize = '20px'
    if (this.lockDelayLimit < 500) {
      $('#delay').classList.add('middelay')
    }
    if (this.lockDelayLimit <= 333) {
      $('#delay').classList.remove('middelay')
      $('#delay').classList.add('lowmiddelay')
    }
    if (this.lockDelayLimit <= 200) {
      $('#delay').classList.remove('lowmiddelay')
      $('#delay').classList.add('lowdelay')
      $('#delay').style.fontSize = (20 + ((200 - this.lockDelayLimit) / 10)) + 'px'
    }
    this.hasLineDelay = false
    this.isDead = false
    ...
    for (let i = 0; i < SPAWN_OFFSETS[rotSys].downShift; i++) {this.shiftDown()}
    ...
    if (settings.settings.IRS === 'hold') {
      if (input.getGameDown('rotateRight') && !input.getGamePress('rotateRight')) this.ire = 1
      else if (input.getGameDown('rotateLeft') && !input.getGamePress('rotateLeft')) this.ire = 3
      else if (input.getGameDown('rotate180') && !input.getGamePress('rotate180')) this.ire = 2
    }
    if (this.ire !== 0) {
  ...
  get yFloor() {return Math.floor(this.y)}
  get visualY() {return (this.y + this.endY)}
  drawMino(x, y, buffer, type, number, color, ctx = this.ctx) {
    ...
    switch (type) {
      ...
      case 'piece':
        let suffix = ''
        if (this.useSpecialI && this.name === 'I') suffix = number
        if (this.useRetroColors) suffix = `-${this.parent.stat.level % 10}`
  ...
  moveValid(passedX, passedY, shape, checkIfFrozen = true) {
    if (this.isFrozen && checkIfFrozen) return false
  ...
  getDrop(distance = (this.parent.settings.height + this.parent.settings.hiddenHeight) * 2) {
    if (this.isStuck) return 0
    let currentDistance = 0
    for (currentDistance = 1; currentDistance <= distance; currentDistance++) {
      if (!this.moveValid(0, currentDistance, this.shape, false)) return currentDistance - 1
    }
    return currentDistance - 1
  }
  checkFall(distance) {
    if (distance < 1) return true
    if (this.getDrop(distance) === Math.floor(distance)) return true
    return false
  }
  get endPoints() {
    if (this.shape == null) {
  ...
  get endX() {return this.endPoints[0]}
  get endY() {return this.endPoints[1]}
  get startPoints() {
    if (this.shape == null) {
      return [0, 0]
    }
  ...
  get startX() {return this.startPoints[0]}
  get startY() {return this.startPoints[1]}
  sonicDrop() {
    this.y += this.getDrop()
    this.isDirty = true
  }
  ...
  addManipulation() {
    if (this.lockdownType !== 'extended') return
    this.manipulations++
    if (this.manipulations === this.manipulationLimit) {
      sound.add('lockforce')
      const cellSize = this.parent.cellSize
  ...
  shiftLeft() {this.shift('x', -1, this.canShiftLeft)}
  shiftRight() {this.shift('x', 1, this.canShiftRight)}
  shiftDown() {this.shift('y', 1, !this.isLanded)}
  killLockDelay() {
    if (this.lockDelay >= this.lockDelayLimit) return
  ...
  rotate(amount, direction, playSound = true) {
    ...
    if (this.killLockDelayOnRotate) this.killLockDelay()
    kickTest:
    for (let i = 0; i <= kickTable.length; i++) {
      if (i === kickTable.length) break // Rotation Failed
  ...
  get hasSpun() {
    if (
      ...
      window.isSpin = true //for dynamic mode
      return true
    } else {
      window.isSpin = false //for dynamic mode
      return false
    }
  }
  get hasSpunMini() {
    if (
      ...
      window.isMini = true //for dynamic mode
      return true
    } else {
      window.isMini = false //for dynamic mode
      return false
    }
  }
  get inAre() {
