...
export default class Stack extends GameModule {
  ...
  makeAllDirty() {
    for (let x = 0; x < this.grid.length; x++) {
      for (let y = 0; y < this.grid[x].length; y++) {this.dirtyCells.push([x, y])}
    }
  }
  gridWithLockdown() {
    ...
    for (const finalBlock of finalBlocks) {
      if (finalBlock[0] < 0 || finalBlock[0] >= this.parent.settings.width || finalBlock[1] >= this.parent.settings.height) {console.log("Piece is too big for Matrix!!")}
  ...
  add(passedX, passedY, shape, color) {
    ...
    sound.syncBgm()
    if (!this.parent.piece.hasHardDropped) {sound.add("locknohd")}
    const checkSpin = this.parent.piece.checkSpin()
    let isSpin = false
    let isMini = false
    if (this.parent.piece.x === this.parent.piece.rotatedX && this.parent.piece.yFloor === this.parent.piece.rotatedY && this.parent.piece.checkSpin().isSpin) {
    ...
    if (this.parent.hold.gainHoldOnPlacement && this.parent.hold.holdAmount < this.parent.hold.holdAmountLimit) {
      this.parent.hold.holdAmount++
      this.parent.hold.isDirty = true
    }
    for (let i = 0; i < this.flashX.length; i++) {this.dirtyCells.push([this.flashX[i], this.flashY[i]])}
    this.flashX = []
    this.flashY = []
    this.flashTime = 0
    ...
    for (let y = 0; y < shape.length; y++) {
      for (let x = 0; x < shape[y].length; x++) {
        ...
        if (isFilled) {
          ...
          if (yLocation - this.hiddenHeight >= 0) {passedLockOut--}
          if (this.parent.piece.useSpecialI && this.parent.piece.name === "I") {this.grid[xLocation][yLocation] = "i" + shape[y][x]}
          else {this.grid[xLocation][yLocation] = color}
    ...
    for (let y = 0; y < this.grid[0].length; y++) {
      for (let x = 0; x <= this.grid.length; x++) {
        if (x === this.grid.length) {
          ...
          for (let x = 0; x < this.grid.length; x++) {delete this.grid[x][y]}
          this.parent.piece.hasLineDelay = true
          this.lineClear++
          this.parent.lineClear++
    ...
    if (this.lineClear > 0) {
      ...
      } else if (this.lineClear < 4) {this.parent.b2b = 0}
      if (this.lineClear < 4) {sound.add(`${type}not4${version}`)}
      else if (type !== "tspin") {
        this.parent.b2b++
        this.parent.maxb2b = Math.max(this.parent.b2b, this.parent.maxb2b)
      }
      ...
      if (this.lineClear > 4) {sound.add(`${b2bPrefix}${type}4${version}`)}
      else {sound.add(`${b2bPrefix}${type}${this.lineClear}${version}`)}
      if (this.parent.b2b > 1) {sound.add("b2b")}
      if (isSpin) {this.parent.addScore(`tspin${this.lineClear}`)}
      if (!pc) {
        if (isSpin) {
          if (this.parent.b2b > 1) {sound.add(`voxb2b_${this.parent.piece.name.toLowerCase()}spin`)}
          else if (isMini) {sound.add(`voxmini${this.parent.piece.name.toLowerCase()}spin`)}
          else {sound.add(`vox${this.parent.piece.name.toLowerCase()}spin${this.lineClear}`)}
        } else {
          if (this.parent.b2b > 1 && this.lineClear === 4) {sound.add("voxb2b_erase4")}
          else {sound.add(`voxerase${this.lineClear}`)}
        }
      }
    } else {
      this.parent.combo = -1
      if (isSpin) {
        if (isMini) {sound.add(`voxmini${this.parent.piece.name.toLowerCase()}spin`)}
        else {sound.add(`vox${this.parent.piece.name.toLowerCase()}spin0`)}
    ...
    if (this.parent.combo > 0) {
      ...
      if (this.parent.combo <= 5) {sound.add("voxren1")}
      else if (this.parent.combo <= 10) {sound.add("voxren2")}
      else {sound.add("voxren3")}
    ...
    } else {$("#combo-counter-container").classList.add("hidden")}
    if (this.parent.piece.areLineLimit === 0 && !settings.settings.stillShowFullActionTextDespiteZeroLineClearAre) {this.collapse()}
    ...
    if (this.useGarbageSending) {
      ...
      if (isSpin && !isMini) {garbageToClear += [0, 2, 3, 4, 4, 4][this.lineClear]}
      if (this.parent.b2b > 1 && this.lineClear) {garbageToClear++}
      const comboIncreaseTable = [2, 5, 7, 9, 12]
      for (const condition of comboIncreaseTable) {
        if (this.parent.combo >= condition) {garbageToClear++}
      }
      if (pc) {garbageToClear += 10}
    }
  ...
  alarmCheck() {
    ...
    if (this.height - this.highest - Math.max(0, this.waitingGarbage) < 2 || (((this.height - this.highest - Math.max(0, this.waitingGarbage) < 5 && !this.alarmIsOn) || (this.height - this.highest - Math.max(0, this.waitingGarbage) < 8 && this.alarmIsOn)) && this.skyToFloor - this.hiddenHeight < this.height - 4)) {this.startAlarm()}
    else {this.endAlarm()}
  }
  updateGrid() {
  ...
  startAlarm() {
    ...
    this.alarmIsOn = true
    this.updateGrid()
    $(".game-center").classList.add("danger")
    $(".stats").classList.add("danger")
  ...
  endAlarm() {
    ...
    this.alarmIsOn = false
    this.updateGrid()
    $(".game-center").classList.remove("danger")
    $(".stats").classList.remove("danger")
  ...
  spawnBrokenLine(amount = 1) {
    ...
    for (let i = 0; i < amount; i++) {
      ...
      //   this.garbageHoleUsed = 0
      // }
      for (let i = 0; i < this.flashY.length; i++) {this.flashY[i]--}
      for (let x = 0; x < this.grid.length; x++) {
        if (this.grid[x][0]) {topOut = true}
        for (let shiftY = 0; shiftY < this.grid[0].length; shiftY++) {this.grid[x][shiftY] = this.grid[x][shiftY + 1]}
        if (
          this.copyBottomForGarbage &&
          !this.grid[x][this.grid[0].length - 2]
        ) {continue}
        if (x === this.garbageRandomHole && !this.copyBottomForGarbage) {continue}
        this.grid[x][this.grid[0].length - 1] = "black"
      }
      if (this.parent.piece.isStuck) {this.parent.piece.y--}
  ...
  collapse() {
    if (this.toCollapse.length === 0) {return}
    let fallenBlocks = 0
    for (const y of this.toCollapse) {
      ...
      for (let i = 0; i < this.flashY.length; i++) {
        if (this.flashY[i] < y) {this.flashY[i]++}
      }
    }
    ...
    if (fallenBlocks !== 0) {
      sound.add("collapse")
      if (this.toCollapse.length >= 4) {sound.add("collapse4")}
      else {sound.add("collapsenot4")}
    }
    this.parent.particle.generate({
      amount: 100,
      x: 0,
