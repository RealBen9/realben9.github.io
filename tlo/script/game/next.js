...
export default class Next extends GameModule {
  ...
  next() {
    this.generate()
    this.isDirty = true
    if (!window.noNext || this.parent.type !== 'mystery') {sound.add(`piece${this.queue[1]}`)}
  ...
  draw() {
    ...
    if (this.nextLength <= 0) {
      ...
      return
    } else {$('#main-next-container').classList.remove('hidden')}
    ...
    if (this.parent.stat.level >= 700 && this.parent.type == 'pandaemonium') {
      ...
      for (let y = 0; y < game.shapedisp.length; y++) {
        for (let x = 0; x < game.shapedisp[y].length; x++) {
          ...
          let suffix = ''
          if (this.parent.piece.useSpecialI && piece === 'I') {suffix = game.shapedisp[y][x]}
          if (this.parent.piece.useRetroColors) {suffix = `-${this.parent.stat.level % 10}`}
          const img = document.getElementById(`mino-${color}${suffix}`)
          const isFilled = game.shapedisp[y][x]
          if (isFilled && !window.noNext) {
      ...
      for (let y = 0; y < shape.length; y++) {
        for (let x = 0; x < shape[y].length; x++) {
          const color = this.parent.colors[piece]
          let suffix = ''
          if (this.parent.piece.useSpecialI && piece === 'I') {suffix = shape[y][x]}
          if (this.parent.piece.useRetroColors) {suffix = `-${this.parent.stat.level % 10}`}
          const img = document.getElementById(`mino-${color}${suffix}`)
    ...
    for (let nextSpace = 0; nextSpace < nextCount; nextSpace++) {
      ...
      if (this.parent.type == 'pandaemonium' && (((this.parent.stat.level >= 500) && (nextSpace == 1)) || ((this.parent.stat.level >= 600) && (nextSpace == 0)))) {
        game.piecedisp = ["T", "J", "Z", "O", "S", "L", "I"][Math.floor(Math.random() * 7)]
        game.shapedisp = PIECES[game.piecedisp].shape[INITIAL_ORIENTATION[this.parent.rotationSystem][game.piecedisp]]
        const offset = this.parent.nextOffsets[game.piecedisp]
        for (let y = 0; y < game.shapedisp.length; y++) {
