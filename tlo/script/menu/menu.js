...
class Menu {
  ...
  load(name, type = 'default', menuData = null) {
    ...
    const render = (menu) => {
      ...
      }, 250) //used to be 500 250 might be faster
    }
    if (menuData != null) {
    ...
    for (let i = 0; i < this.current.data.length; i++) {
      ...
      switch (currentData.type) {
        case 'overline':
          ...
          break
        case 'header':
          window.label = currentData.label
          element = document.createElement('header')
        ...
        case 'toggle':
          ...
          if (window.label == "Sudden" && currentData.string == "ruleOptionLabel") {
            element.classList.add('sudden')
          }
          break
        case 'select':
          element = document.createElement('div')
...
`<div class='set-control'>ArrowUp ✕ </div><div class='set-control'>ArrowUp ✕ </div><div class='set-control'>+</div>`
      ...
      if (currentData.type === 'control') {
        ...
        if (currentData.action === "settingsimport") {
        ...
        } else if (currentData.action === "replayimport") {
          const inputThingy = document.createElement("input")
          inputThingy.id = "import-replay"
          inputThingy.type = "file"
          inputThingy.accept = ".tlorep"
          inputThingy.classList.add("input-thingy")
          element.appendChild(inputThingy)
          inputThingy.addEventListener("change", (event) => {
            const theFile = event.target.files[0]
            ...
            reader.addEventListener('load', (event) => {
              ...
              const repData = readData.split("\n")
              window.replaying = true
              window.repInputData = JSON.parse(repData[12])
              window.repInputs = []
              for (i = 0; i < window.repInputData.length; i+=3) {
                if (window.repInputs[window.repInputData[i]] == null) {window.repInputs[window.repInputData[i]] = []}
                window.repInputs[window.repInputData[i]].push(window.repInputData[i+1],window.repInputData[i+2])
              }
              console.log(window.repInputs)
              gameHandler.newGame(repData[2], repData[11]-0)
            })
            reader.readAsText(theFile)
          })
          const labelForInputThingy = document.createElement("label")
          labelForInputThingy.setAttribute("for", "import-replay")
          labelForInputThingy.innerText = currentData.label
          labelForInputThingy.classList.add("label-for-input-thingy")
          element.appendChild(labelForInputThingy)
        }
  ...
  ok() {
    if (this.selectedData.submenu == "mode/nontwo") {
    ...
    switch (this.selectedData.action) {
      ...
      case 'settingsimport':
        sound.playMenuSe("optionselect")
        // Look at line 453 for logic
        break
      case 'replayimport':
        sound.playMenuSe("optionselect")
        // Look at line 476 for logic
        break
      case 'settingsexport':
        sound.playMenuSe("optionselect")
