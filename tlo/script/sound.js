...
class Sound {
        ...
        .then((soundData) => {
          ...
          if (settings.settings.voicebank !== 'off' && ( settings.settings.voicebank == 'talonen' || settings.settings.voicebank == 'talonde' ) ) {
            for (const voxName of [
              ...
              'sspin0', 'sspin1', 'sspin2', 'sspin3', 'sspin4', 'minisspin', 'b2b_sspin', 
              'ispin0', 'ispin1', 'ispin2', 'ispin3', 'ispin4', 'ispin5', 'miniispin', 'b2b_ispin', 
          ...
          if (
            ...
            for (const piece of Object.keys(PIECES)) {
              ...
              this.sounds[soundName] = new Howl({
                ...
                onend: () => {
                  this.amountOfTimesEnded[soundName]++
                  if (this.amountOfTimesEnded[soundName] > 5 && this.playingSeLoops[soundName] != null && this.fadedSounds[soundName] == null) {
                    this.fadedSounds[soundName] = true
                    this.sounds[soundName].fade(settings.settings.sfxVolume / 100, (settings.settings.sfxVolume / 100) * 0.5, 500)
                  }
                },
  ...
  loadBgm(name, type) {
    if (settings.settings.musicVolume == 0) {return}
